...
## Permettre de modifier dynamiquement un paragraphe ou son contenu à chaque ajout ou modification de paragraphe
Par exemple pour mettre des valeurs au premier item d'un paragraphe multivalué au moment de son ajout dans le back-office :

```
/**
 * We modify the default value of the first 'lieu_experience' paragraph added to the node.
 */
function customization_field_widget_entity_reference_paragraphs_form_alter(&$element, \Drupal\Core\Form\FormStateInterface $form_state, $context)
{
  //Kint::dump($element['subform']['field_lieu']['widget']);
  if (
      $element['#paragraph_type'] !== 'lieu_experience' ||
      $element['#weight'] > 0 ||
      !empty($element['subform']['field_description']['widget'][0]['value']['#default_value'] !== '')
    ){
    return;
  }
  $element['subform']['field_description']['widget'][0]['value']['#default_value'] = 'Votre hôte vous accueille';
  $element['subform']['field_lieu']['widget']['#default_value'][] = '6';
}
```

## Permettre de faire dépendre la valeur d'un paragraphe en fonction de la valeur issue d'un autre paragraphe

Ici lorsqu'on choisit un pays, la liste des états dans un autre paragraphe est mise à jour (à l'aide d'une fonction ajax)
...
function modulename_field_widget_entity_reference_paragraphs_form_alter(&$element, &$form_state, $context)
{
    if ($element['#paragraph_type'] == 'country_select')
    {
        ...
        // Add ajax
        $element['subform']['field_country']['widget']['#ajax'] = ['callback' => 'update_state_dropdown', 'event' => 'change', 'method' => 'html', 'wrapper' => $gen_id, 'progress' => ['type' => 'throbber', 'message' => NULL, ], ];
